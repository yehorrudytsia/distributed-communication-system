...
module.exports = app => {
  ...
  const start = (req, userId) => {
    ...
    app.storage.setSession({ userId, token, data })
    return session;
  }
  ...
  const restore = async req => {
    ...
    if (!session) {
      const [record] = await app.storage.getSession(token)
      if (record.data) {
  ...
  const getUser = login => app.storage.getUser(login)

  const getUserId = token => app.storage.getUserId(token)

  const registration = user => app.storage.registration(user)
